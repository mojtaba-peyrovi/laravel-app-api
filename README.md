...
in this case we can specify which methods we need to use. like this:
```
...
```
the migration of category_product_table makes a table called category_table  we need to use the names singular as models are not tables which are plural. for example we don't say categories_products.
- inside the pivot migration we just need to add the foreign keys of both models.


## part 10:
- in order to use one of the existing users lets say, to use them in seller_id factory we can say like this:
 ```
...
- in order to empy the pivot table we have to disable foreign key checks like this:
...
```
factory(User::class, $usersQuantity);  
```
...
factory(Product::class, $usproductsQuantity)->create()->each(
 ...
 ```           
**_this is so important:_**

when I was running the seeder, I got the error that seller table doesnt exist. and we shouldnt have a seller table as it is a subst of users table.
in order to get around this error we need to manually specify in Seller model that the table name is user. like this:
```
protected $table = 'users';
```
we also do this for buyers.
- we can run the migration and seeder two ways :
```
1) php artisan migrate:refresh    and then     php artisan db:seed     
```
